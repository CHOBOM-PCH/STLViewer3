	...
	if (Now) {
		...
		if (Now->line == line) {
			if (Now->linebefore == -100) {
				start->Set(Now->mm[0], Now->mm[1], Now->mm[2]);
				while (1) {
					if (Now->linenext == -100) {
...
int CCalc::FindLine(int line, int * sharp, int remove)
{
	int k, count=0, run = 1;
	bool ret;
	GoFirst((CNode*&)Solid->Face);
