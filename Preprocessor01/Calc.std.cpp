...
bool CCalc::ReadStl(char* name, int mode, double offset)
{
	...
	float v[3][3];
	float n[3];
	int  ii=0, jj=0, count = 0, k, index=0, rev;
	int  facet_num=0;
				...
				if (Idx->seq == 0 && Fdx->id != Idx->id) {
					if (fdir == 1) {
						...
						*/
						...
						s.Set(Fdx->mf[0], Fdx->mf[1], Fdx->mf[2]);
						...
						ret = Near(&s, &t0, &t1, &t2, &t3);     // mf 검사
						if (ret) {
							if ((Fdx->d[0]*Idx->mm[0] + Fdx->d[1]*Idx->mm[1] + Fdx->d[2]*Idx->mm[2] + Fdx->d[3]) >= 0) {     // mf 검사
								fdir = 1;
							} else {
								ret = 0;
								fdir = 1;
							...
							MB:
							s.Set(Fdx->mb[0], Fdx->mb[1], Fdx->mb[2]);
							ret = Near(&s, &t0, &t1, &t2, &t3);     // mb 검사
							if (ret) {
								if ((Fdx->d[0]*Idx->mm[0] + Fdx->d[1]*Idx->mm[1] + Fdx->d[2]*Idx->mm[2] + Fdx->d[3]) < 0) {     // mb 검사
									fdir = 1;
								} else {
									ret = 0;
									fdir = 1;
				...
				if (Idx->seq == 0 && Bdx->id != Idx->id) {
					if (bdir == 1) {
						...
						*/
						...
						s.Set(Bdx->mb[0], Bdx->mb[1], Bdx->mb[2]);
						t0.Set(Idx->v0->x, Idx->v0->y, Idx->v0->z);
						t1.Set(Idx->v1->x, Idx->v1->y, Idx->v1->z);
						t2.Set(Idx->v2->x, Idx->v2->y, Idx->v2->z);
						t3.Set(Idx->mf[0], Idx->mf[1], Idx->mf[2]);
						//ret = Near(&s, &t0, &t1, &t2, &t3);     // mb 검사
						s.Set(Bdx->mf[0], Bdx->mf[1], Bdx->mf[2]);     // mf 검사
						ret = Near(&s, &t0, &t1, &t2, &t3);
						if (ret) {
							if ((Bdx->d[0]*Idx->mm[0] + Bdx->d[1]*Idx->mm[1] + Bdx->d[2]*Idx->mm[2] + Bdx->d[3]) >= 0) {     // mf 검사
							//if ((Bdx->d[0]*Idx->mm[0] + Bdx->d[1]*Idx->mm[1] + Bdx->d[2]*Idx->mm[2] + Bdx->d[3]) < 0) {     // mb 검사
								bdir = -1;
							} else {
								ret = 0;
								bdir = -1;
							...
							MF:
							s.Set(Bdx->mb[0], Bdx->mb[1], Bdx->mb[2]);     // mb 검사
							//s.Set(Bdx->mf[0], Bdx->mf[1], Bdx->mf[2]);     // mf 검사
							ret = Near(&s, &t0, &t1, &t2, &t3);
							if (ret) {
								if ((Bdx->d[0]*Idx->mm[0] + Bdx->d[1]*Idx->mm[1] + Bdx->d[2]*Idx->mm[2] + Bdx->d[3]) < 0) {     // mb 검사
